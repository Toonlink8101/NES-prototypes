...
reset:
    ...
    lda #$40
    sta $4017 ; diable APU IRQ (frame counter)
    cld       ; disale decimal mode
    ldx #$FF
    txs       ; init stack
    ...
    ; Now waiting for PPU to stabilize (~30,000 cycles)
    ...
    ; enable the NMI for graphical updates, and jump to main
    lda #%10001000
    sta $2000
    jmp main
...
nmi_ready:       .res 1 ; set to push a PPU frame update, 2 to turn rendering off next NMI
...
.segment "OAM"
oam: .res 256 ; sprite OAM data to be sent via DMA
...
nmi:
    ...
    bne :+ ; nmi_ready == 0, not ready to update PPU
        jmp @ppu_update_end
    :
    cmp #2 ; nmi_ready == 2 turns off rendering
    bne :+
        lda #%00000000
        sta $2001
        ldx #0
...
@scroll:
    lda scroll_nmt
    and #%00000011 ; only keep 2 lowest bits to prevent error
    ora #%10001000
    sta $2000
    lda scroll_x
    sta $2005
...
; ppu_off: waits until next NMI, turns off rendering (now safe to write PPU directly via $2007)
ppu_off:
    lda #2
    sta nmi_ready
    :
...
; ppu_update_byte: like ppu_update_tile, bbut X/Y makes the high/low bytes of the PPU address to write
